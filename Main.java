...
import javax.swing.JTextField;
    ...
    public class Main extends JFrame implements KeyListener {
        ...
        public Main() {
            grids = new JTextArea [30] [15]; // Устанавливаем строку и столбец игровой области
            data = new int [30] [15]; // Открытие пространства массива данных согласуется со строками и столбцами игровой области
        ...
        public void init() {
            ...
            center.setLayout (new GridLayout (30, 15, 1, 1)); // делим строки и столбцы на основную область игры, всего 26 строк и 12 столбцов
            for (int i = 0; i <grids.length; i ++) {// инициализируем панель
                for (int j = 0; j < grids[i].length; j++) {
                    grids[i][j] = new JTextArea(19, 18);
            ...
            right.add(new JLabel(" g : left        j : right"));
            right.add(new JLabel(" h : down   y : change"));
            right.add(label);
            label1.setForeground (Color.GREEN); // Установите для содержимого метки красный шрифт
            right.add(label1);
            ...
            this.setSize (900, 850); // Устанавливаем размер окна
    ...
    public static void main(String[] args) {
        ...
        public void start() {
            ...
            for (int i = 0; i <30; i ++) {// Всего 26 слоев, падающих один за другим
                try {
                    ...
                    if (canFall (x, y) == false) {// Если его нельзя отбросить
                        ...
                        for (int k = x; k <x + 4; k ++) {// Пройдите по 4 слоям, чтобы увидеть, есть ли квадраты в каждом слое, чтобы удалить этот ряд квадратов и подсчитать результат
                            ...
                            if (sum == 13) {// Если в слое k есть блоки, удалите блоки в слое k
                                removeRow(k);
                            }
                        }
        ...
        public void draw(int m, int n){
            ...
            for (int i = 0; i < 4; i++) {// Проходим через 16 квадратов (4 * 4)
                for (int j = 0; j < 4; j++) {
                    if ((temp & rect)!= 0) {// Когда здесь квадрат
                        grids [m] [n].setBackground(Color.RED);// Место с квадратами становится зеленым
                    }
                    n ++; // Следующий столбец
                    temp >>= 1;
                }
        ...
        public void keyTyped(KeyEvent e) {
            if(e.getKeyChar () == 'g') { // Перемещаем квадрат влево
                if (running == false) {
                    return;
                }
            ...
            if (e.getKeyChar() == 'j') {// Квадрат перемещается вправо
                if (running == false) {
                    return;
                }
                int temp = 0x8000;
            ...
            if (e.getKeyChar() == 'h') {// Блок перемещается вниз
                if (running == false) {
                    return;
                }
            ...
            if (e.getKeyChar() == 'y') {// Изменение формы поля
                if (running == false) {
                    return;
                }
                int i = 0;
